...
# PREPARATION. 
#####################################################################################################################
rm(list=ls())                                                                       # 1/06/2020 Eva Silarova unified upper and lower cases in variable names - v1.1
...
for (j in 1:numberSpeciesStratumCombinations) {
  ...
  counts <- read.table(paste(arguments$File, "_counts.csv", sep = ""), header = TRUE,dec=".",sep=";")   #13/01/2023 Javier Rivas: Changes to produce outputs with dots - v1.6
...
write.table(all_Indices_All_Trends, "All_Indices_All_Trends.csv", row.names = FALSE,sep=";",dec=".")           # 09/04/2021 Eva Silarova changed "all" to capital "All": "all_Indices_All_Trends.csv" was changed to "All_Indices_All_Trends.csv" - v1.3
