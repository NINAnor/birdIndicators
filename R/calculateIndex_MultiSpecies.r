...
#path_TrimFiles <- working_folder_rel
#Spp_subset <- sppLists$sppLists$MSI_farmland

calculateIndex_MultiSpecies <- function(path_TrimFiles, Spp_subset, IndexName){
  ...
  ## Check whether all desired species are present in data
  if(!(all(Spp_subset %in% CombinedIndex_sel$Code))){
    missingSPP <- Spp_subset[which(!(Spp_subset %in% CombinedIndex_sel$Code))]
    warning(paste0("The following species in Spp_subset are not available in the data: "))
