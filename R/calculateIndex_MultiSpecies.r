...
setwd("P:/12580000_tov_fauna/TOV-E/2023_MSI_Chloe/Species_files_2022")
# DPJ: In this folder you must have the species csv files from the rtrim-shell:
# - stratum
# - output
# - fitted values
# - TT
# - ocv
# plus the COMB_Trimind (below)


## Load the indices
CombinedIndex <- read_csv2('COMB_Trimind.csv') 
...
CombinedIndex$Index <- as.numeric(CombinedIndex$Index)
CombinedIndex$Index_SE <- as.numeric(CombinedIndex$Index_SE)




## Getting the indicator species of each habitat
AgricSpecies <- c(4930,
                  5410,
                  9760,
                  9920,
...
CombinedIndex_sel <- CombinedIndex %>%
  mutate(Indicator = case_when(Code %in% AgricSpecies ~ 'Agricultural',
                               TRUE ~ 'NA' )) %>%
  filter(Indicator == 'Agricultural')

CombinedIndex_sel %>% distinct(Code)


AgricFiles <- NULL

for(i in 1:length(AgricSpecies)){
  
  AgricFiles[i] <- list.files(pattern = paste0(AgricSpecies[i], '_1_63_indices_TT.csv'))
}
...
for(i in 1:length(MountSpecies)){
  
  MountFiles[i] <- list.files(pattern = paste0(MountSpecies[i], '_1_63_indices_TT.csv'))
  
}


ForestSpecies <- c(10090,11220,10840,12000,14610,
                   13140,14540,17100,15390,8630,
                   12020,14860,8760,12590,13120,
                   12760,12770,16360,12010,13110,
                   13350,11870,14420, 10990)

tibble(ForestSpecies = ForestSpecies) %>% arrange(ForestSpecies) %>% print(n =25)


CombinedIndex_sel2 <- CombinedIndex %>%
  mutate(Indicator = case_when(Code %in% ForestSpecies ~ 'Forest',
                               TRUE ~ 'NA' )) %>%
  filter(Indicator == 'Forest')

CombinedIndex_sel2 %>% distinct(Code) %>% print(n=25)
# not in the list: 8630, 12020, 14860 -> data only from 2008
...
for(i in 1:length(ForestSpecies)){
  ...
  ForestFiles[i] <- list.files(pattern = paste0(ForestSpecies[i], '_1_63_indices_TT.csv'))
...
AgricSpecies.dat <- rbindlist(lapply(AgricFiles, fread), idcol = "origin")
AgricSpecies.dat[, origin := factor(origin, labels = basename(AgricFiles))]


AgricSpecies.dat2 <- AgricSpecies.dat %>%
  tibble() %>%
...
AgricSpecies.dat3 <- AgricSpecies.dat2 %>%
  select(species, year, index, se)

write.csv(AgricSpecies.dat3, 'Agricultural_indicators_2022.csv', row.names = F)
write.table(AgricSpecies.dat3, 'Agricultural_indicators_2022.txt', row.names = F)



ForestSpecies.dat <- rbindlist(lapply(ForestFiles, fread), idcol = "origin")
ForestSpecies.dat[, origin := factor(origin, labels = basename(ForestFiles))]


ForestSpecies.dat2 <- ForestSpecies.dat %>%
  tibble() %>%
  select(origin, Year, Index_model, Index_model_SE) %>%  
  rename(year = Year,
         index_ori = Index_model,
...
ForestSpecies.dat3 <- ForestSpecies.dat2 %>%
  select(species, year, index, se)


write.csv(ForestSpecies.dat3, 'Forest_indicators_2022_incomplete.csv', row.names = F)
write.table(ForestSpecies.dat3, 'Forest_indicators_2022.txt', row.names = F)




MountainSpecies.dat <- rbindlist(lapply(MountFiles, fread), idcol = "origin")
MountainSpecies.dat[, origin := factor(origin, labels = basename(MountFiles))]


MountainSpecies.dat2 <- MountainSpecies.dat %>%
  tibble() %>%
  select(origin, Year, Index_model, Index_model_SE) %>%  
  rename(year = Year,
         index_ori = Index_model,
...
MountainSpecies.dat3 <- MountainSpecies.dat2 %>%
  select(species, year, index, se) 

MountainSpecies.dat3 %>%
  filter(year == 2008)


write.csv(MountainSpecies.dat3, 'Mountain_indicators_2022.csv', row.names = F)
write.table(MountainSpecies.dat3, 'Mountain_indicators_2022.txt', row.names = F)
...
setwd("P:/12580000_tov_fauna/TOV-E/2023_MSI_Chloe/Species_files_2022") # set working directory for input and output files
rdata <- read.table("Forest_indicators_2022_NEW.txt", header=TRUE) 		 # name of input file
jobname <- as.character ("Forest_indicators_2022_NEW") 			# generic name for output files
...
changepoint <- 2008 					# compare trends before and after this year
