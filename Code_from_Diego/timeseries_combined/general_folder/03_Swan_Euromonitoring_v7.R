...
Check_steering_files <- function(general_folder_path) {
  ...
  if (file.exists("Schedules.csv")  == TRUE)     { 
    ...
    schedules_file                  <- read.csv2("Schedules.csv", header=TRUE,check.names = FALSE)   
  ...
  if (file.exists("Pools.csv")      == TRUE)     { 
    ...
    pools_file                      <- read.csv2("Pools.csv", header=TRUE,check.names = FALSE)    
  ...
  if (file.exists("Countries.csv")  == TRUE)     { 
    ...
    countries_file                  <- read.csv2("Countries.csv", header=TRUE,check.names = FALSE)    
  ...
  if (file.exists("Species.csv")    == TRUE)     { 
    ...
    species_file                    <- read.csv2("Species.csv", header=TRUE,check.names = FALSE)   
  ...
  if (file.exists("Species_countries.csv")  == TRUE)  { 
    ...
    species_countries_file                  <- read.csv2("Species_countries.csv", header=TRUE,check.names = FALSE)
  ...
  if (file.exists("Swan_schedules.csv")  == TRUE)     { 
    ...
    Swan_schedules  <- read.csv2("Swan_schedules.csv", header=TRUE,check.names = FALSE) 
    ...
    if (file.exists("Swan_function_list.csv")== TRUE)  { 
      Swan_function_list     <- read.csv2("Swan_function_list.csv", header=TRUE)                # function list
    ...
    write.csv2(Swan_schedules, file = "Swan_schedules.csv", row.names=FALSE) # properly sorted 
...
Run_Swan <- function(general_folder, working_folder)     { 
  ###################################################################################################
  ...
  Swan_schedules          <<- read.csv2(Swan_schedules_name, header=TRUE,check.names = FALSE)   
  ...
  Swan_function_list      <<- read.csv2("Swan_function_list.csv", header=TRUE)    
  ...
  countries_file          <<- read.csv2("Countries.csv", header=TRUE,check.names = FALSE)    
  species_file            <<- read.csv2("Species.csv", header=TRUE,check.names = FALSE)   
  species_countries       <<- read.csv2("Species_countries.csv", header=TRUE,check.names = FALSE)
...
Select_all_available_datafiles <- function(no_arguments) { 
  ...
  setwd(general_folder) 
  write.csv2(Swan_schedules, file = "Swan_schedules.csv", row.names=FALSE)
  Swan_schedules          <<- read.csv2("Swan_schedules.csv", header=TRUE,check.names = FALSE)   
  setwd(working_folder) 
...
Check_content_datafiles <- function(no_arguments) { 
  ...
  for (r in 1:nrow(Swan_schedule_active)) { # Note: loop follows the records in Swan_schedule
    ...
    TimetotalsStratum  <- read.csv2(RTRIM_timetotals, header=TRUE)
    ...
    TimeocvStratum  <- read.csv2(RTRIM_ocv, header=TRUE)
    ...
    TimeargStratum     <- read.csv2(RTRIM_arguments, header=TRUE)
  ...
  for (s in 1:nspecies) { 
    ...
    for (p in 1:npools) { 
      ...
      for (st in 1:nstrata) { 
        ...
        TimetotalsStratum  <- read.csv2(filenameTimeTotals_out, header=TRUE)
...
Truncate_datafiles <- function(no_arguments) { 
  ...
  for (st in 1:nrecords) { 
    ...
    if (nrows_before > nrows_after) { # truncation has occurred
      ...
      write.csv2(TimetotalsStratum, file=filenameTimeTotals_out, row.names=FALSE)
      
      #     also truncate RTRIM ocv file:
      filenameCovmatrix <- paste(species_nr, "_",  1, "_",  stratum_nr, "_ocv.csv", sep="")  # level1 = country file input
      ocvStratum <- read.csv2(filenameCovmatrix, header=TRUE)
      ...
      write.csv2(ocvStratum, file=filenameCovmatrix, row.names=FALSE)
...
Strip_ocv_files <- function(ocvfiles_to_strip) { 
  ...
  for (i in 1:length(ocvfiles_to_strip)) {
    ...
    if (!is.na(ocvfiles_to_strip[i])) { 
      ocvtest <- read.csv2(ocvfiles_to_strip[i],header=TRUE) # read ocv
    ...
    ###########################################################################   
    write.csv2(ocvtest, file = ocvfiles_to_strip[i], row.names=FALSE) 
  ...
  for (level in 2:nlevels) { 
    ...
    for (s in 1:nspecies) { 
      ...
      for (p in 1:npools) { 
        ...
        write.csv2(TimetotalsAllStrata,paste0(working_folder,"\\",all_species[s], "_",  level-1, "_","TimetotalsAllStrata.csv"))
        ...
        for (st in 1:nstrata) {
          ...
          icv_1site = unname(as.matrix(read.csv2(filenameCovmatrix,header=TRUE))) # unname is required to delete colnames and rownames
        ...
        if (class(result) == "trim") { 
          ...
          if(TimetotalsAllStrata$time< base_year){        #Javi 21.09.2021
          ...
          write.csv2(tt_indices, file=filenameTimeTotals_out, row.names=FALSE)
          ...
          write.csv2(FI, filenameFittedValues_out, row.names = FALSE)
          # ARCO 9-10-2021    
          ...
          write.csv2(covmatrix,file=filenameOcv_out, row.names=FALSE)
          ...
          if (exists("arg_output_file") == TRUE)  {
            ...
            arg_output_file$Changepoints                 <- paste(result$changepoints, collapse = ", ")    # Changepoints used to calculate the indices.
            ...
            write.csv2(arg_output_file, filenameArg_out, row.names = FALSE)
          } 
...
Gather_results_of_countries_horizontal_format  <- function(no_arguments) {
  ...
  write.csv2(Swan_total, file = Swan_countries_name, row.names=FALSE)  
...
Gather_results_of_countries_vertical_format  <- function(no_arguments) {
  ...
  write.csv2(trimind_countries, file = "Trimind_countries.csv", row.names=FALSE)  
  write.csv2(trimtot_countries, file = "Trimtot_countries.csv", row.names=FALSE)  
...
Gather_results_of_pools_horizontal_format <- function(years) { 
  ...
  print(paste0("Este es el las year in data ",last_year_in_data))
  nyear <- length(uyear)
  ...
  if (file.exists(Swan_countries_name))  { 
    Swan_countries <- read.csv2(Swan_countries_name, header=TRUE,check.names = FALSE)   
  ...
  write.csv2(Swan_total, file = Swan_name, row.names=FALSE)  
  ...
  write.csv2(trimslo_from, file = trimslo_name, row.names=FALSE)  
  setwd(working_folder) 
...
Gather_results_of_pools_vertical_format <- function(years) {                              # Arco 19-12-2020  
  ...
  if (file.exists("Trimind_countries.csv"))  { 
    trimind_countries <- read.csv2("Trimind_countries.csv", header=TRUE,check.names = FALSE)   
  ...
  if (file.exists("Trimtot_countries.csv"))  { 
    trimtot_countries <- read.csv2("Trimtot_countries.csv", header=TRUE,check.names = FALSE)   
  ...
  write.csv2(trimind_total, file = trimind_name, row.names=FALSE)  
  write.csv2(trimtot_total, file = trimtot_name, row.names=FALSE)  
  setwd(working_folder) 
