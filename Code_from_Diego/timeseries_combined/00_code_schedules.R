...
#wd <- "C:/Users/diego.pavon-jordan/OneDrive - NINA/Documents/NINA_ONGOING_projects/2023_Pipeline_for_data_access/For_Chloe/Combining_time_Series"
#setwd(paste0(wd, '/NorwayOld_converted'))


path.TT.data = paste0(wd,"/general+working folder/working_folder/")
schedule.path= paste0(wd,"/general+working folder/general_folder/")


## Each species has 3 files of data associated, you just need one of them to avoid repetitions, so select indices_TT files
pattern=(".*\\_indices_TT.csv")
vector_files=list.files(path.TT.data,pattern= pattern)
vector_files # All the file names. 110 files. 55 from old, 55 from new scheme
...
#This loop selects only those files at national level, and save the species code and the country code in its correct position
